...
/* Note regarding ldesk_* namespace: this avoids potential conflicts
     ...
     size_t len;
{
	...
	if ( t ) {
		...
		memcpy(p, buf, t);
		ldesk_MD5Transform (ctx->buf, ctx->in);
		buf += t;
		len -= t;
	}
...
main (int argc, char **argv)
{
	...
	for (j = 1; j < argc; ++j)
	{
		...
		ldesk_MD5Update (&context, argv[j], strlen (argv[j]));
